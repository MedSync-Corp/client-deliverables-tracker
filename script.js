// script.js — full app: dashboard + clients + client-detail + overrides + negatives + lifetime + Started tags + recommendations
...
/* ========== Shared DOM handles (present only where relevant) ========== */
// Dashboard bits
...
function overrideForWeek(overrideRows, clientId, refMon) {
  const hit = (overrideRows || []).find(
    r => r.client_fk === clientId && String(r.week_start).slice(0,10) === refMon.toISOString().slice(0,10)
  );
...
async function openClientModalById(id) {
  ...
  // preserve original start date shown on edit
...
/* ========== Create/Update Client (addresses, EMRs, baseline commitment) ========== */
clientForm?.addEventListener('submit', async (e) => {
  ...
  // baseline weekly commitment (upsert if changed)
  const { qtyEl, startEl } = weeklyEls();
...
/* ========== Dashboard ========== */
function computeRowsForDashboard(clients, wk, ovr, comps) {
  const today = new Date(); today.setHours(0,0,0,0);
  const mon = mondayOf(today); const fri = fridayEndOf(mon);
  ...
  return { rows, today: today, mon, fri };
}
...
async function loadDashboard() {
  ...
  const { rows, today, mon, fri } = computeRowsForDashboard(clients, wk, ovr, comps);
  ...
  if (btnReco) {
    btnReco.onclick = () => openRecommendation(rows, today);
  }
}
...
function renderDueThisWeek(rows) {
  ...
  if (!items.length) {
    dueBody.innerHTML = `<tr><td colspan="6" class="px-4 py-6 text-gray-500">No active commitments this week.</td></tr>`;
    return;
  }
  dueBody.innerHTML = items.map(r=>{
    const done = Math.max(0, r.required - r.remaining);
    return `<tr>
      <td class="px-4 py-2"><a class="text-indigo-600 hover:underline" href="./client-detail.html?id=${r.id}">${r.name}</a></td>
      <td class="px-4 py-2">${fmt(r.required)}</td>
      <td class="px-4 py-2">${fmt(done)}</td>
      <td class="px-4 py-2">${fmt(r.remaining)}</td>
      <td class="px-4 py-2"><status-badge status="${r.status}"></status-badge></td>
      <td class="px-4 py-2 text-right"><button class="px-2 py-1 rounded bg-gray-900 text-white text-xs" data-log="${r.id}" data-name="${r.name}">Log</button></td>
    </tr>`;
  }).join('');
  // log clicks
  dueBody.onclick = (e)=> {
...
/* ========== Recommendation planner (dashboard) ========== */
function weekdayLabelsFrom(today) {
  // Mon..Fri, but only from "today" (inclusive) to Fri
  const mon = mondayOf(today);
  const fri = fridayEndOf(mon);
  const labels = [];
  const d = new Date(today); d.setHours(0,0,0,0);
  while (d <= fri) {
    labels.push(d.toLocaleDateString(undefined, { weekday:'short' })); // e.g., Mon, Tue
    d.setDate(d.getDate()+1);
  }
  return labels;
}
// Evenly split each client's remaining across remaining weekdays, round to integers, sum of row equals remaining
function splitEven(remaining, days) {
  if (days <= 0) return [];
  const base = Math.floor(remaining / days);
  const rem = remaining - base * days;
  const arr = new Array(days).fill(base);
  for (let i = 0; i < rem; i++) arr[i] += 1; // distribute remainder
  return arr;
}
function openRecommendation(rows, today) {
  if (!recoModal || !recoHead || !recoBody) return;
  const labels = weekdayLabelsFrom(today);
  ...
  recoHead.innerHTML = `
    <tr>
      ...
      ${labels.map(l=>`<th class="px-3 py-2 text-right">${l}</th>`).join('')}
      <th class="px-3 py-2 text-right">Total</th>
    </tr>
  `;

  // body
  const rowsWithWork = rows.filter(r => r.remaining > 0);
  if (!rowsWithWork.length) {
    recoBody.innerHTML = `<tr><td class="px-3 py-3 text-gray-500" colspan="${days+2}">Nothing remaining this week 🎉</td></tr>`;
  } else {
    recoBody.innerHTML = rowsWithWork.map(r => {
      const plan = splitEven(r.remaining, days);
      const t = plan.reduce((a,b)=>a+b,0);
      return `<tr>
        <td class="px-3 py-2">${r.name}</td>
        ${plan.map(n=>`<td class="px-3 py-2 text-right">${fmt(n)}</td>`).join('')}
        <td class="px-3 py-2 text-right font-medium">${fmt(t)}</td>
      </tr>`;
    }).join('');
  }

  // show
  recoModal.classList.remove('hidden');
}
function closeRecommendation() { recoModal?.classList.add('hidden'); }
recoClose?.addEventListener('click', closeRecommendation);
recoOk?.addEventListener('click', closeRecommendation);

/* ========== Log modal (shared) — allows negatives with confirm ========== */
...
async function loadClientsList() {
  ...
  if (!supabase) { clientsTableBody.innerHTML = `<tr><td class="py-4 px-4 text-sm text-gray-500">Connect Supabase (env.js).</td></tr>`; return; }
...
async function loadClientDetail() {
  ...
  if (contact) contact.innerHTML = client?.contact_email
  ...
  setTxt('carryIn', fmt(carryIn));
  setTxt('required', fmt(required));
  setTxt('done', fmt(doneThis));
  setTxt('remaining', fmt(remaining));
  ...
  if (canv && window.Chart) {
    ...
    window.__clientChart = new Chart(canv.getContext('2d'), {
      type:'bar',
      data:{ labels:['This week'], datasets:[{
        label:'Remaining',
        data:[point],
        backgroundColor:(ctx)=>ctx.raw.color,
        hoverBackgroundColor:(ctx)=>ctx.raw.hover,
  ...
  if (body) {
    ...
    const remLast = Math.max(0, targetLast - doneLast); // last week row shows raw shortfall
    body.innerHTML = [
      rowHtml(lastMon, baseLast, oLast ?? null, targetLast, doneLast, remLast),
...
function openOverrideModal(clientId, weekStartISO) {
  ...
  ovrForm.week_start.value = weekStartISO; // yyyy-mm-dd (Monday)
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Safe calls: each loader no-ops if not on that page.
  await loadDashboard();
  await loadClientsList();
  await loadClientDetail();

  // expose for inline ‘Log’ button on client row
